...
def bridge_clusters(tri, bad, span):
    """Split violating facets into ones that bridge and ones that cantilever.

    A flat ceiling supported on both sides is a bridge, which a printer handles;
    one hanging off a single edge is not. Facets are grouped by shared vertices
    and a group counts as a bridge when neither of its horizontal extents
    exceeds `span` - a short ceiling has walls close by on both sides, a long
    one does not.
    """
    idx = np.flatnonzero(bad)
    if len(idx) == 0:
        return np.zeros(len(tri), bool), []
    key = np.round(tri[idx].reshape(-1, 3) / 1e-4).astype(np.int64)
    ...
    for a, b, c in inv:
        for x, y in ((a, b), (b, c)):
            ...
            if ra != rb:
                parent[rb] = ra
    groups = {}
    for n, f in enumerate(inv):
        groups.setdefault(find(f[0]), []).append(idx[n])
    bridged = np.zeros(len(tri), bool)
    report = []
    for members in groups.values():
        pts = tri[members].reshape(-1, 3)
        lo, hi = pts.min(axis=0), pts.max(axis=0)
        ext = hi - lo
        axis = 0 if ext[0] <= ext[1] else 1      # the short way is what gets spanned
        short = ext[axis]
        mid = (lo + hi) / 2
        # a bridge has wall on both sides of the short axis: probe just outside
        # each end, at the height of the ceiling, and ask whether that is solid
        sides = []
        for sgn in (-1, 1):
            probe = mid.copy()
            probe[axis] = (lo[axis] if sgn < 0 else hi[axis]) + sgn * 0.35
            probe[2] = mid[2] - 0.2
            sides.append(point_inside(probe, tri))
        is_bridge = short <= span and all(sides)
        if is_bridge:
            bridged[members] = True
        report.append((is_bridge, short, max(ext[0], ext[1]), mid, all(sides)))
...
def check(tri, max_overhang, angle_tol=1.0, area_tol=0.05, bed_tol=0.01, probe=0.05,
    ...
    res["bridges"] = []
    if bridge_span > 0:
        bridged, groups = bridge_clusters(tri, bad, bridge_span)
        res["bridges"] = [g for g in groups if g[0]]
        res["bridge_area"] = float(area[bridged].sum())
        bad = bad & ~bridged
...
def main():
    ...
    ap.add_argument("--bridge-span", type=float, default=0.0,
                    help="treat ceilings no wider than this in mm as bridges, not overhangs")
    args = ap.parse_args()
    try:
        ...
        print(f"ERROR: {exc}")
        return 2
    r = check(tri, args.max_overhang, args.angle_tol, args.area_tol, args.bed_tol,
              args.probe, args.bridge_span)
    lo, hi = r["bbox"]
    ...
    if args.bridge_span > 0:
        print(f"bridges allowed   {len(r['bridges'])} up to {args.bridge_span:.1f} mm span, "
              f"{r.get('bridge_area', 0.0):.2f} mm^2")
        for ok, short, long_, c, walls in r["bridges"][:6]:
            print(f"    spans {short:.1f} mm, {long_:.1f} long, walls both sides at "
