...
BRIDGE_SPAN = 0.0   # set from --bridge-span in main, read by run() in each worker
...
def run(job, bridge_span=0.0):
    ...
    if res["ok"]:
        ...
        bridged = r.get("bridge_area", 0.0)
        lo, hi = r["bbox"]
        res.update(result="PASS" if r["pass"] else "FAIL", worst=r["worst_overhang"],
                   p2=r["p2_area"], bridged=bridged, floating=len(r["p3_floating"]),
...
def main():
    ...
    ap.add_argument("--bridge-span", type=float, default=0.0,
                    help="ceilings no wider than this count as bridges, not overhangs "
                         "(the strap channel's groove in the bed face is one)")
    a = ap.parse_args()
    global BRIDGE_SPAN
